...
def create_order():
    ...
    if request.method == 'POST':
        ...
        with Session_db() as cursor:
            new_order = Orders(order_list = basket,
...
def order_details(order_id):
    with Session_db() as cursor:
        ...
        total_price = sum([cursor.scalar(select(Menu).filter_by(name = name_product)).price * int(cnt) for name_product, cnt in us_order.order_list.items()])
